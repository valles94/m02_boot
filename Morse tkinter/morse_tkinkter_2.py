...
class Translator(ttk.Frame): # la clase Translator hereda de "ttk.Frame", que es un rectangulo del tamaño que yo quiera, y modificable
    def __init__(self, padre, **kwargs): #esta funcion, nos pide siempre el "padre", que significa a donde voy a anclar el rectangulo que creo con el "ttk.Frame" y el parametro "**kwargs" que es un diccionario, es decir los aceptara siempre y cuando tengan clave y valor
        ...
        self.origin_text.bind("<KeyRelease>", self.modificaOrigin) # la funcion "bind" nos permite crear un evento para poder asignar diferentes acciones a las variables, en este caso a self.origin_text, "KeyReleas" nos va a permitir que no haga falta tener que pulsar el boton de traduccion para traducir, si no qeu se vaya traduciendo a medida que se escribe en la caja de texto, luego llamamos a nuestra funcion de traduce para que haga efecto 
    ...
    # No haría falta crear esta funcion si añadiesmos a la funcion "traduce" un argumento mas que fuese el receptor de la letra al escribir, en este caso, habria que añadir argumento +  None: ejemplo: "letra = None", ya que si no lo ponemos, aunque se haga traduuccion inmediata, si queremos darle al boton traduce daria un error porque le estamos pasando argumentos que no puede recibir, pero si le damos valor None, le decimos que puede aceptar o no argumento extra. la hemos creado para ver mas claro el ejemplo.
    def modificaOrigin(self, letra): # Creamos una funcion que llame a la funcion "traduce" pero esta funcion sirve para colocarla en el "bind" de KeyRelease para la traduccion inmediata, con dos argumentos, el self propio y uno mas que es la letra que recibirá al escribir
        self.traduce() # llamamos a la funcion traduce


    def chageText(self):
        #texto_original = self.origin_text.get("1.0", END) # OPCIONAL asignamos a una variable el texto original y con el metodo .get("1.0", END) le decimos que nos coja desde el indice 0 hasta el final del texto para traducir
        #texto_destino = self.destino_text.get("1.0", END)  # OPCIONALasignamos a una variable el texto destino y con el metodo .get("1.0", END) le decimos que nos coja desde el indice 0 hasta el final del texto para traducir
        if self.traduccionDirecta == True: # este es el interruptor que habiamos configurado al principio nos permite modificar para hacer el cambio
            ...
            # traduccion = morse_2.to_texto(texto_destino) # OPCIONAL creamos la variable traduccion que es equivalente al modulo importado "morse_2" dentro de ese llamamos a su funcion "t_texto" ala que le introducimos la variable del texto original que hemos creado antes
        else:
            self.origin_lbl.config(text = "Plano") 
            self.destino_lbl.config(text = "Morse") 
            # traduccion = morse_2.to_morse(texto_original) # OPCIONAL
        self.destino_text.delete("1.0", END) # con este aprametro borramos el texto de destino cada vez que le damos a "intercambiar" 
        #self.destino_text.insert(INSERT, texto_original) # OPCIONAL con este prametro, insertamos el texto que habiamos traducido del cuadro destino al cuadro origen
        self.origin_text.delete("1.0", END) # con este aprametro borramos el texto de origen cada vez que le damos a "intercambiar"
        #self.origin_text.insert(INSERT, texto_destino) # OPCIONAL con este prametro, insertamos el texto que habiamos traducido del cuadro origen al cuadro destino
    ...
    def send_telegram(self):
        sender = self.sender.get() # creamos una variable que recoja la informacion del sender
        receiver = self.receiver.get() # creamos una variable que recoja la info del receiver
        if self.traduccionDirecta: # si la traduccion es True:
            msg = self.origin_text.get("1.0", END) # se almacena en la variable msg todo el contenido del cuadro de texto a traducir
        else:
            msg = self.destino_text.get("1.0", END) # si es false, se almacenara en msg el contenido inverso
        
        if sender.strip() == "": # aqui le decimos que si el recuadro de sender, esta vacio, que nos lance un aviso de que hay que rellenarlo
            messagebox.showwarning("Aviso", "Debe informar el sender", parent = self) # la funcion "messaagebox.showwarning" es para sacar una ventana pop up con mensaje de aviso, dentro lde damos los parametros wque quremos que nos diga, y el "parent = self" sirve para que la ventana pop up salga en la misma ventana que hemos creado de la app y no en cualquier parte de lap antalla
            return
        if receiver.strip() == "": # aqui le decimos que si el recuadro de receiver, esta vacio, que nos lance un aviso de que hay que rellenarlo
            messagebox.showwarning("Aviso", "Debe informar el receiver", parent = self)
            return
        if msg.strip() == "": # aqui le decimos que si el recuadro de mensaje a traducir, esta vacio, que nos lance un aviso de que hay que rellenarlo
